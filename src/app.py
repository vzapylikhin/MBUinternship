...
df = pd.read_excel("Наставники.xlsx", usecols=[2,3,4,5,6,7])
df = pd.DataFrame(df.values, columns=['Выберите своего наставника', 'Заинтересованность','Коммуникабельность', 'Компетентность',
                                      'Подача информации','Обратная связь'])
for i in df.columns[1:]:
    df[i] = df[i].astype('int')
data = pd.read_excel('наставники1.xlsx')

blok = data[data['ФИО'].isin(df['Выберите своего наставника'].unique())]["Блок"].unique()
otdel = data[data['ФИО'].isin(df['Выберите своего наставника'].unique())]['Отдел'].unique()
fio = data[data['ФИО'].isin(df['Выберите своего наставника'].unique())]["ФИО"].unique()
...
dct_pos = {'Кабанова Юлия Николаевна': 'Начальник отдела информационно-аналитического обеспечения',
 ...
 'Тоноян Грант Егишевич': 'Начальник отдела ведения нормативно-справочной информации',
 'Логинова Елена Александровна': 'Начальник отдела организации интеграционного взаимодействия',
 'Пишков Алексей Алексеевич': 'Начальник отдела информационно-технологического сопровождения централизованной бухгалтерии',
 'Гридасов Максим Александрович': 'Начальник административного отдела',
 'Струкова Анастасия Анатольевна': 'Начальник отдела расчетов, обязательств и государственного долга',
 'Храмов Павел Владимирович': 'Начальник отдела централизованного учета доходов',
 'Амерханова Эльвира Гумеровна': 'Начальник отдела централизованного учета капитальных вложений и НИОКР',
 ...
 'Прищеп Оксана Сергеевна': 'Начальник отдела консолидированной бюджетной отчетности и анализа',
 'Чижов Сергей Сергеевич': 'Начальник отдела консолидированной бухгалтерской отчетности подведомственных учреждений и анализа',
 'Мосина Нина Алексеевна': 'Начальник отдела централизованного учета операций с межбюджетными трансфертами',
 ...
 'Ставрунова Виктория Зиновьевна': 'Начальник отдела централизованного учета расчетов и обязательств',
 'Королева Юлия Вячеславовна': 'Начальник отдела централизованного начисления заработной платы и иных выплат',
 'Монашев Пётр Игоревич': 'Начальник отдела внутреннего контроля и аудита',
 'Жучков Павел Анатольевич': 'Начальник отдела централизованного ведения Главной книги',
 'Самокиш Ариадна Артуровна': 'Начальник юридического отдела',
 'Попова Ольга Николаевна': 'Начальник отдела государственной гражданской службы и кадров'}
                ...
                html.Table([
                    ...
                    html.Tbody([
                        html.Tr([
                            ...
                            html.Td(
                                html.H6(round(rating.iloc[0, 2], 2),
                                        style={'textAlign': 'left',
                                               'color': 'white',
                                               'margin-top': '10px',
                                               'fontSize': 20,
                        ...
                        html.Tr([
                            ...
                            html.Td(
                                html.H6(round(rating.iloc[1, 2], 2),
                                        style={'textAlign': 'left',
                                               'color': 'white',
                                               'margin-top': '10px',
                                               'fontSize': 20,
                        ...
                        html.Tr([
                            ...
                            html.Td(
                                html.H6(round(rating.iloc[2, 2], 2),
                                        style={'textAlign': 'left',
                                               'color': 'white',
                                               'margin-top': '10px',
                                               'fontSize': 20,
                        ...
                        html.Tr([
                            ...
                            html.Td(
                                html.H6(round(rating.iloc[3, 2], 2),
                                        style={'textAlign': 'left',
                                               'color': 'white',
                                               'margin-top': '10px',
                                               'fontSize': 20,
                        ...
                        html.Tr([
                            ...
                            html.Td(
                                html.H6(round(rating.iloc[4, 2], 2),
                                        style={'textAlign': 'left',
                                               'color': 'white',
                                               'margin-top': '10px',
                                               'fontSize': 20,
...
def get_options(blok):
    ter = data[(data['ФИО'].isin(df['Выберите своего наставника'].unique()))&(data["Блок"] == blok)]
...
def get_options_1(otdel):
    ter1 = data[(data['ФИО'].isin(df['Выберите своего наставника'].unique()))&(data['Отдел'] == otdel)]
